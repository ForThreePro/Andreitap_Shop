...
function cargarSeccionesExtras() {
    ...
    if(paisActual === 'mx'){
        ...
        streamings.forEach(s => {
            grid.innerHTML += `<div class="card"><h3>${s.titulo}</h3>${s.items.join('<br>')}<button class="btn-comprar" onclick="contactarProducto('${s.titulo}', 'ver precio', true)">Comprar</button></div>`;
        });

        const docDiv = document.getElementById('documentos');
        docDiv.innerHTML = '';
        documentos.forEach(d => {
            docDiv.innerHTML += `<div class="producto-card"><div><b>${d}</b></div><button class="btn-comprar" onclick="contactarProducto('${d}', '$45', true)">Comprar</button></div>`;
        });

        const recDiv = document.getElementById('recargas');
        recDiv.innerHTML = '';
        recargas.forEach(r => {
            if(r.includes("PRIMERO")){
                recDiv.innerHTML += `<p style="text-align:center; margin:10px 0; font-size:0.85rem; font-style:italic;">${r}</p>`;
            } else {
                recDiv.innerHTML += `<div class="producto-card"><div><b>${r}</b></div><button class="btn-comprar" onclick="contactarProducto('${r}', 'ver precio', true)">Comprar</button></div>`;
            }
        });
...
function cargarMetodos() {
    ...
    metodos.forEach(m => { metodosDiv.innerHTML += `<span class="metodo">${m}</span>`; });
}
...
function contactarProducto(producto, precio, esCompra) {
    ...
    let paisTxt = document.getElementById('selectorPais').options[document.getElementById('selectorPais').selectedIndex].text;
